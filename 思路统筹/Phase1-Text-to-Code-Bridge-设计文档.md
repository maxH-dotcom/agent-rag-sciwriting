...
| 研究问题解析范围 | 主效应优先 | MVP 仅稳定支持单一因变量 + 少量自变量 |
| 数据输入范围 | 单 sheet CSV 优先 | 暂不处理多 sheet 选择和跨 sheet 拼接 |
| 变量类型识别 | 依赖用户描述 | 暂不自动严格区分连续/二值/计数变量 |

### 2.1 MVP 约束说明

Text-to-Code Bridge 在 MVP 阶段不是一个通用统计顾问，而是一个受约束的“证据驱动代码生成器”。因此这里显式采用以下假设：

1. 用户问题默认可被压缩为主效应任务。
   - 优先支持 `"X 对 Y 的影响"`、`"Y 与 X 的关系"` 这一类表达。
   - `"GDP and FDI jointly on CO2 controlling for trade openness"` 这类复合问题，后续版本再引入变量角色解析器。
2. 上传数据默认是单表输入。
   - 推荐使用单 sheet CSV。
   - Excel 多 sheet 暂不做 sheet 选择 UI，也不做自动主表识别。
3. 变量类型依赖显式描述。
   - 若用户未说明变量是连续、二值还是计数，Bridge 不基于列值分布强行推断模型。
   - 需要通过任务描述或中断确认补齐。
...
class CodeGenRequest(BaseModel):
    ...
    data_files: List[str]                    # 数据文件路径（MVP 默认单 sheet CSV）
...
class GeneratedCode(BaseModel):
    ...
    # 新增字段（2026-04-04）：适应性解释
    adaptation_explanation: str = Field(
        default="",
        description="方法适配说明：解释代码的核心逻辑、针对用户数据的调整点、调整的依据"
    )
```
...
async def generate_code_node(state: TextToCodeState) -> TextToCodeState:
    ...
    # 获取迁移上下文（如果有）
    transfer_context = state.get("transfer_context", {})
    ...
    迁移上下文（来自 Novelty Node）：
    - 来源方法：{transfer_context.get('source_method', '未指定')}
    - 需要的调整：{transfer_context.get('required_adaptations', [])}
    - 变量映射：{transfer_context.get('variable_mapping', {})}
    - 风险提示：{transfer_context.get('adaptation_risk', '无')}
    ...
    6. 【新增】生成适应性解释（adaptation_explanation）：说明
       - 代码的核心逻辑是什么
       - 针对用户数据做了哪些调整
       - 调整的依据是什么
    ...
    {{
        ...
        "adaptation_explanation": "适应性解释：说明代码逻辑、数据调整点、调整依据"
    }}
    """
